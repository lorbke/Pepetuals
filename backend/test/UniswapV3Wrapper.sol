...
contract UniswapV3WrapperTest is Test {
	...
	function testCreateLpAndCollateralPools() public {
		...
		uint32 period = 1;
		long = mlsp.getPoolLongCollat(period);
		short = mlsp.getPoolShortCollat(period);
		require (long != address(0), "long pool should not be zero");
		require (short != address(0), "short pool should not be zero");
    ...
    function testProvideLiquidity() public {
		long = mlsp.getPoolLongCollat(1);
		short = mlsp.getPoolShortCollat(1);
