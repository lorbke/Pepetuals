...
const approveOrder = () => {
	...
	// 	console.log(test);
	// });
	ctrusdc.approve(JSON.parse(contractData.body)[state.chainId.toString()].contractAddress, parseInt(state.input) * 1e6).then((test) => {
		console.log(test);
		ctr.buy({name: ethers.utils.formatBytes32String(state.selectedOutput), long: state.long, period: state.perpetual == true ? 4294967295 : 1, leverage: state.leverage}, parseInt(state.input) * 1e6).then((test) => {
			console.log(test);
		});
	});
};
